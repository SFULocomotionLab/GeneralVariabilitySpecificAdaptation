function [SF,tsave] = SFcalc(names, Fs, t, allDatai, jj, ii, tri)
    
    % Use GRF to calculate SF for now because I am not sure how foot
    ...
    % refilter the COP data from 0.3-3 Hz -- this taken from Shawn O'Connor
    n = 3;
    Wn = [0.3 3]/(Fs/2);
    ...
    for r = 2:length(copy)
        if r == step_save(e)
            ...
            % by 60 to get bpm
            SF(e) = (1/diff([t(step_save(e-1)) t(step_save(e))]))*60;
    ...
    if 0
        ...
        if strcmp(tri,'ga1') || strcmp(tri,'ga2') || strcmp(tri,'nw1') || strcmp(tri,'nw2')
            ...
            hold on
            plot(tsave,SF,'Color',[col 0.5])
            plot([tsave(1) tsave(end)],[nanmean(SF) nanmean(SF)],'-k')
            ylim([80 140])
