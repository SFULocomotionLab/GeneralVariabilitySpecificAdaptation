function [L_AnkleRange, R_AnkleRange, L_ankleAngle_timelock_resampled, R_ankleAngle_timelock_resampled, L_gaitcycle, R_gaitcycle] = AnkleAnglePeak(names, Fs, allDatai, jj, ii, tri,tauL)
    ...
    for tLi = 1:tLength-1
        if ~isempty(TO1(TO1 > HS1(tLi) & TO1 < HS1(tLi+1))) % step
            ...
            if length(L_ankle_init) > lowerLim && length(L_ankle_init) < upperLim && Fz1(HS1(tLi)+100) > mean(Fz1)
                L_init(tLii) = mean(L_ankle_init(1));
                L_ankle_init = Lankleangle(HS1(tLi):HS1(tLi+1)) - L_init(tLii);
...
%                 L_AnkleRange(tLii) = range([temp2 temp3]);
                L_AnkleRange(tLii) = temp3; % peak ankle extension angle

                % log angle
                L_ankleAngle_timelock(1:L_length(tLii),tLii) = L_ankle_init;
                L_ankleTime_timelock(1:L_length(tLii),tLii) = time(HS1(tLi):HS1(tLi+1)) - time(HS1(tLi));
    ...
    for tRi = 1:tRength-1
        if ~isempty(TO2(TO2 > HS2(tRi) & TO2 < HS2(tRi+1)))
            ...
            if length(R_ankle_init) > lowerLim && length(R_ankle_init) < upperLim && Fz2(HS2(tRi)+100) > mean(Fz2)
                R_init(tRii) = mean(R_ankle_init(1));
                R_ankle_init = Rankleangle(HS2(tRi):HS2(tRi+1)) - R_init(tRii);
...
%                 R_AnkleRange(tRii) = range([temp2 temp3]);
                R_AnkleRange(tRii) = temp3;  % peak ankle extension angle

                % log angle
                R_ankleAngle_timelock(1:R_length(tRii),tRii) = R_ankle_init;
                R_ankleTime_timelock(1:R_length(tRii),tRii) = time(HS2(tRi):HS2(tRi+1)) - time(HS2(tRi));
